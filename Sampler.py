...
class Sampler(object):
    ...
    def __init__(self, undersamplingType, percentOfKSpace, centrePercent, stepsize, lines2ignore, maxAmplitude4PDF, ROdir, noOfSpokes, fullresSpokesMulFactor, interpolationSize, sliceShape=None):
        assert undersamplingType in list(range(7 + 1)) + list(
            range(10, 11 + 1)
        ), 'Unrecognized undersamplingType'
        ...
        self.isRadial = undersamplingType >= 10
        self.dummySlice = None if sliceShape is None else np.zeros(sliceShape)
    ...
    def calculateSamplings(self, slice=None, returnMeta=False):
        ...
        if self.undersamplingType == 0: #Varden1D
            ...
            metaname = 'PDF'
            samplingname = f'VardenMask1D_percent{str(self.percentOfKSpace)}_maxamp{str(self.maxAmplitude4PDF)}_rodir{str(self.ROdir)}'
            ...
            metaname = 'PDF'
            samplingname = f'VardenMask2D_percent{str(self.percentOfKSpace)}_maxamp{str(self.maxAmplitude4PDF)}_centrePercent{str(self.centrePercent)}'
            ...
            samplingname = f'CenterRatioMask_percent{str(self.percentOfKSpace)}'
        elif self.undersamplingType == 6: #CenterSquareMask
            data = createCenterSquareMask(slice, self.percentOfKSpace, returnMeta)
            metaname = 'NumOfLinesRemoved'
            samplingname = f'CenterSquareMask_percent{str(self.percentOfKSpace)}'
            ...
            metaname = 'PDF'
            samplingname = f'#HighFreqMask_percent{str(self.percentOfKSpace)}_maxamp{str(self.maxAmplitude4PDF)}_rodir{str(self.ROdir)}'

        elif self.undersamplingType == 8: #Varden2Dv0
            data = createVardenMask2Dv0(slice, self.percentOfKSpace, returnMeta)
            metaname = 'PDF'
            samplingname = f'VardenMask2Dv0_percent{str(self.percentOfKSpace)}'
            ...
            if not bool(self.noOfSpokes):
                ...
                samplingname = f'GoldenAngle_dynspokes{str(noOfSpokes)}_percent{str(self.percentOfKSpace)}_fulResMulFact{str(self.fullresSpokesMulFactor)}'

                omtuple, dcftuple = createGASampling(slice, noOfSpokes, self.fullresSpokesMulFactor, returnFullOM=True, returnInvOM=True)
            else:
                samplingname = f'GoldenAngle_spokes{str(self.noOfSpokes)}_fulResMulFact{str(self.fullresSpokesMulFactor)}'
